        ...
        for (const [code, t] of tvs) {
          if (t.tvRowId === resultado.tv.id) {
            ...
            if (t.ws.readyState === 1) {
              ...
              t.ws.send(JSON.stringify({ type: "controller_status", online: false }));
            }
          }
        }
        broadcastTvList();
...
// Recalcula e reenvia o status de "controlador online" pra cada TV PAREADA
// conectada agora. Chamado sempre que algum controlador entra ou sai do ar
// — precisa recalcular por TV (não é mais um heartbeat global) porque cada
// TV só se importa com os controladores da PRÓPRIA conta (+ ADM). TVs ainda
// não pareadas (conta_id null) não têm "controlador da conta" nenhum —
// ficam mostrando o código de pareamento até alguém reivindicar.
function notifyAllTvs() {
  tvs.forEach((tv) => {
    if (tv.contaId == null) return;
    if (tv.ws.readyState === 1) {
      tv.ws.send(JSON.stringify({ type: "controller_status", online: controllersOnlineParaConta(tv.contaId) }));
    }
  });
}
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "tv_connect") {
      ...
      const registrarSemPareamento = () => {
        ...
        ws.send(JSON.stringify({ type: "controller_status", online: false }));
      };
      ...
      tvsPareamento.buscarOuCriarPorDeviceId(deviceId, nomeSugerido).then((registro) => {
        ...
        ws.send(JSON.stringify({
          ...
          online: paired ? controllersOnlineParaConta(registro.conta_id) : false,
        }));
        broadcastTvList();
      }).catch((err) => {
