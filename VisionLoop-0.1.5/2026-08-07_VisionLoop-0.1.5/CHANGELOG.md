...
Funcionalidade nova: **validade opcional para cada mídia enviada**.

### O que mudou

- **Todo envio agora pergunta se o conteúdo deve ter prazo.** Ao escolher ou arrastar arquivos, aparece uma janela com duas opções: *Sem validade* (padrão — o arquivo fica até você excluir à mão) ou *Expirar em uma data e hora*, que abre um campo de data com horário. A pergunta é feita uma vez por envio e vale para todos os arquivos daquela leva; cancelar a janela cancela o envio.
- Vale para as **três zonas de envio**: a de "Transmitir para todas", a do painel da TV e a de dentro do formulário de playlist.
- **Ao vencer, o arquivo é removido sozinho** — apagado do armazenamento (disco local ou bucket R2) e retirado de todas as playlists que o usavam. Playlist que fica sem nenhum item é excluída junto, como já acontecia na exclusão manual.
- **Selo de validade nos cartões de mídia**, mostrando data e hora do vencimento. Ele fica em âmbar quando falta menos de 24 horas, para o prazo não pegar ninguém de surpresa.
- A data escolhida é convertida para UTC antes de ir ao servidor, então o prazo vale independentemente do fuso de quem enviou. Data no passado é recusada com aviso na própria janela.

### Detalhes técnicos

- Nova rota `GET /media-meta` com as validades vigentes. Ela existe separada de propósito: `/videos-list` continua devolvendo um array simples de nomes, que é o formato consumido em vários pontos do controlador — mudar essa resposta quebraria a listagem em cascata.
- As validades ficam em `media-meta.json`, no mesmo modelo do `playlists.json`.
- `expiresAt` é aceito nas duas rotas de envio: `POST /upload-video` (modo disco) e `GET /request-upload` (modo R2, onde o servidor registra o prazo no momento em que assina a URL, já que o arquivo em si nunca passa por ele).
- A limpeza roda na inicialização do servidor (pegando o que venceu enquanto ele esteve fora do ar) e depois a cada minuto. Além disso, `/videos-list` já filtra o que venceu — então mesmo entre uma varredura e outra nada expirado aparece na tela.

### Verificação

Testado num navegador de verdade: envio sem validade (nenhum selo, arquivo permanece); envio com validade (selo com a data correta, registro gravado no servidor); recusa de data no passado; a janela aparecendo também no envio de dentro do formulário de playlist, com o arquivo entrando normalmente na ordem de reprodução; selo em âmbar para prazo menor que 24h; e cancelar a janela não enviando nada. O ciclo completo de vencimento foi verificado ponta a ponta: com o prazo vencido, o arquivo sumiu de `/videos-list` na hora, e na varradura seguinte foi apagado do disco, retirado da playlist que o usava e removido do registro.

### Atenção

Assim como o `playlists.json`, o `media-meta.json` fica no disco do Render, que é efêmero — **as validades se perdem num redeploy**, mesmo com o R2 configurado (os arquivos sobrevivem, os prazos não). Resolver isso pede mover esses dois arquivos para armazenamento persistente, o que continua em aberto.

---

## 0.1.4-fix — 07/08/2026

Correção: **não dava para enviar arquivos de dentro do formulário de playlist**.

### O que estava acontecendo

O formulário de playlist só tinha a grade "Vídeos disponíveis — clique para adicionar", que lista o que já foi enviado antes. Não havia nenhuma zona de upload ali. Com a biblioteca vazia, dava para preencher o nome da playlist e mais nada: era preciso sair do formulário, subir os arquivos em outro lugar da tela e voltar para montar a playlist.

### Corrigido

- **Zona de envio dentro do formulário de playlist**, logo acima da grade de disponíveis, aceitando clique ou arrastar — os mesmos formatos do resto do app (`.mp4`, `.jpg`, `.png`, `.webp`).
- **O que sobe por ali já entra na ordem de reprodução automaticamente**, sem precisar clicar de novo na grade. Imagens entram com o tempo padrão de 10s, pronto para ajustar.
- O item usa o **nome final devolvido pelo servidor**, e não o nome do arquivo no computador. Isso importa porque, quando já existe um arquivo com aquele nome, o servidor salva como `Nome (1).png` — usar o nome original faria a playlist apontar para o arquivo errado. Vale para os dois modos de envio: pelo servidor (disco local) e direto pro Cloudflare R2.
- O envio continua sequencial, um arquivo por vez, com a mesma barra de progresso das outras zonas.

### Verificação

Testado num navegador de verdade, começando com a biblioteca **vazia** (o cenário relatado): abrir "Nova Playlist", enviar dois arquivos pela zona de dentro do formulário e ver os dois entrarem sozinhos na ordem de reprodução, com campo de tempo; alterar um tempo para 18s, salvar, e conferir no `playlists.json` que gravou `18` e o padrão `10` no outro. Testado também o caso de nome repetido: enviando um arquivo cujo nome já existia, a playlist recebeu corretamente `Promo (1).png` — o nome renomeado pelo servidor — e não o original. Nenhum erro de JavaScript no console.

---

## 0.1.4 — 07/08/2026

Remoção da aba "Vídeos" do controlador, a pedido: as duas áreas agora convivem numa tela só.

### O que mudou

- **A barra de abas ("Vídeos" | "Playlists") deixou de existir.** O controlador passou a ser uma tela única e rolável: em cima fica a área de mídia e controle da TV (o que era a aba "Vídeos") e, logo abaixo de um separador, o gerenciador de playlists.
- **Nenhuma funcionalidade foi perdida ou movida de lugar no código.** Todos os ids, handlers e funções de renderização continuam idênticos — envio de arquivos, grade de mídias, botões de reproduzir/pausar/parar/tela cheia, "Transmitir para todas", criação e edição de playlists e envio de playlist para a TV seguem funcionando exatamente como antes.
- A função `switchTab()` foi removida do `js/controller.js` (nada mais a chamava) e as regras `.tabs`/`.tab-btn` saíram do `css/controller.css`, inclusive as da versão para celular.
- Ajuste de rolagem: antes cada painel rolava por dentro (`.content` com `overflow: hidden` e painéis com `flex: 1`). Como agora os dois ficam empilhados, quem rola é a área de conteúdo inteira — `.no-selection` e `.panel` deixaram de esticar, e `.playlist-manager` passou a ter altura mínima própria em vez de depender de `height: 100%` do painel-pai.
- README atualizado: as duas menções à "aba Vídeos"/"aba Playlists" viraram referências à tela única.

### Verificação

Testado num navegador de verdade, com controlador e TV em abas separadas: criar playlist com imagem e tempo personalizado (22s), salvar, enviar para a TV e ver a TV exibindo; selecionar a TV, abrir o painel de controle, mandar um vídeo avulso, pausar e parar — tudo chegando na TV. Nenhum erro de JavaScript no console. Layout conferido em 1440px e em 390px (celular), sem vazamento horizontal e com o gerenciador de playlists mantendo altura utilizável nos dois.
