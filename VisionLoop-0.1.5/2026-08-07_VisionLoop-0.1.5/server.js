...
// ---------- Validade da mídia ----------
// Cada arquivo pode ter (opcionalmente) uma data/hora de expiração. Ao subir,
// quem envia escolhe: sem validade (o padrão — fica até ser excluído à mão) ou
// com validade. Guardamos isso num arquivo à parte para NÃO mudar o formato de
// /videos-list, que é um array simples de nomes e é lido em vários pontos do
// controlador. Formato: { "arquivo.mp4": { "expiresAt": "2026-09-01T12:00:00.000Z" } }
const MEDIA_META_FILE = path.join(__dirname, "media-meta.json");
...
function loadMediaMeta() {
  ...
  try { return JSON.parse(fs.readFileSync(MEDIA_META_FILE, "utf8")); }
  catch { return {}; }
}

function saveMediaMeta(data) {
  fs.writeFileSync(MEDIA_META_FILE, JSON.stringify(data, null, 2), "utf8");
}

// Aceita a data que veio na requisição só se for uma data real e no futuro.
// Devolve a data normalizada em UTC (ISO) ou null quando não há validade.
function normalizeExpiry(raw) {
  if (!raw) return null;
  const t = Date.parse(raw);
  if (!Number.isFinite(t)) return null;
  if (t <= Date.now()) return null; // já venceu: trata como "sem validade"
  return new Date(t).toISOString();
}

function setMediaExpiry(name, expiresAt) {
  const meta = loadMediaMeta();
  if (expiresAt) meta[name] = { expiresAt };
  else delete meta[name];
  saveMediaMeta(meta);
}

function removeMediaMeta(name) {
  const meta = loadMediaMeta();
  if (name in meta) { delete meta[name]; saveMediaMeta(meta); }
}
...
function expiredNames(meta) {
  return Object.keys(meta || {}).filter((name) => isExpired(meta[name]));
}

// Apaga um arquivo do armazenamento ativo (bucket R2 ou disco local).
function deleteMediaFile(name) {
  if (R2_ENABLED) return deleteFromR2(name);
  const target = path.join(__dirname, "videos", name);
  const safeRoot = path.resolve(path.join(__dirname, "videos")) + path.sep;
  if (!path.resolve(target).startsWith(safeRoot)) return Promise.reject(new Error("caminho inválido"));
  return new Promise((resolve, reject) => {
    fs.unlink(target, (err) => {
      // Arquivo já inexistente não é erro aqui: o objetivo (sumir) foi atingido.
...
// Remove tudo que passou da validade: apaga o arquivo, tira das playlists que
// o usavam e limpa o registro. Roda na inicialização e de minuto em minuto.
async function sweepExpiredMedia() {
  const meta = loadMediaMeta();
  const vencidos = expiredNames(meta);
  if (!vencidos.length) return [];
  for (const name of vencidos) {
    try { await deleteMediaFile(name); }
    catch (err) { console.error(`Falha ao excluir mídia vencida "${name}":`, err); }
    removeMediaFromPlaylists(name);
    removeMediaMeta(name);
    console.log(`🗓️  Mídia vencida removida: ${name}`);
  }
  return vencidos;
}
...
const server = http.createServer((req, res) => {
  ...
  if (urlPath === "/videos-list") {
    if (R2_ENABLED) {
      listR2Media()
        .then((files) => {
          // Esconde na hora o que já passou da validade, mesmo que a varredura
          // periódica ainda não tenha rodado — o usuário nunca vê um item
          // vencido "ainda vivo" na tela.
          const meta = loadMediaMeta();
          const visiveis = files.filter((f) => !isExpired(meta[f]));
          res.writeHead(200, { "Content-Type": "application/json" });
          res.end(JSON.stringify(visiveis));
        })
        .catch((err) => {
    ...
    const files = allFiles.filter(
      (f) => !f.startsWith(".") && MEDIA_EXT_REGEX.test(f) && !isExpired(meta[f])
    );
    res.writeHead(200, { "Content-Type": "application/json" });
  ...
  // Validade de cada mídia, num endpoint separado justamente para não mudar o
  // formato de /videos-list (array de nomes), que o controlador já consome em
  // vários lugares. Devolve só o que ainda está valendo.
  if (urlPath === "/media-meta") {
    const meta = loadMediaMeta();
    const vivos = {};
    for (const name of Object.keys(meta)) {
      if (!isExpired(meta[name])) vivos[name] = meta[name];
    }
    res.writeHead(200, { "Content-Type": "application/json" });
    res.end(JSON.stringify(vivos));
    return;
  }
  ...
  if (req.method === "GET" && urlPath === "/request-upload") {
    ...
    // Validade opcional escolhida no momento do envio (vazio = sem validade).
    const expiresAt = normalizeExpiry(reqUrl.searchParams.get("expiresAt"));
    ...
    wouldExceedStorageCap(incomingBytes).then((overCap) => {
      ...
      getUniqueFilename(null, safeName).then((finalName) => {
        ...
        getSignedUrl(s3Client, command, { expiresIn: 300 })
          .then((url) => {
            // O arquivo vai direto do navegador pro bucket, então o servidor
            // registra a validade aqui, junto com o nome que autorizou.
            if (expiresAt) setMediaExpiry(finalName, expiresAt);
            res.writeHead(200, { "Content-Type": "application/json" });
            res.end(JSON.stringify({ url, filename: finalName, contentType, expiresAt: expiresAt || null }));
          })
          .catch((err) => {
  ...
  if (req.method === "POST" && req.url.startsWith("/upload-video")) {
    ...
    getUniqueFilename(dir, safeName).then((finalName) => {
      ...
      writeStream.on("finish", () => {
        if (failed) return;
        if (expiresAt) setMediaExpiry(finalName, expiresAt);
        respondUploadSuccess(res, destPath, finalName, { expiresAt: expiresAt || null });
      });
...
// Limpeza da mídia vencida: uma vez ao subir (pega o que venceu enquanto o
// servidor esteve fora do ar) e depois de minuto em minuto. A validade também
// é conferida em /videos-list, então mesmo entre uma varredura e outra o
// usuário nunca vê na tela algo que já passou do prazo.
const SWEEP_INTERVAL_MS = 60 * 1000;
sweepExpiredMedia().catch((err) => console.error("Erro na varredura inicial de validade:", err));
setInterval(() => {
  sweepExpiredMedia().catch((err) => console.error("Erro na varredura de validade:", err));
}, SWEEP_INTERVAL_MS);
