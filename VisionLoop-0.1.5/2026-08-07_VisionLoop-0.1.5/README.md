...
**Versão 0.1.5** — 07/08/2026 · O histórico de mudanças fica em [CHANGELOG.md](CHANGELOG.md).
...
Na tela do controlador, clique na zona de upload ou arraste os arquivos:
...
### Validade opcional (data para o conteúdo sair do ar)

A cada envio o VisionLoop pergunta se aquele conteúdo deve ter prazo:

- **Sem validade** (padrão) — o arquivo fica no ar até você excluir manualmente.
- **Expirar em uma data e hora** — ao chegar o prazo, o arquivo é apagado sozinho do armazenamento e retirado de todas as playlists que o usavam. Playlist que ficar sem nenhum item é excluída junto.

A pergunta é feita uma vez por envio e vale para todos os arquivos daquela leva. Nos cartões da biblioteca, quem tem prazo mostra um selo com a data — em âmbar quando falta menos de 24 horas.

⚠️ As validades ficam no arquivo `media-meta.json`, no disco do servidor. Como o disco do Render é efêmero, **os prazos se perdem num redeploy** (os arquivos no R2 sobrevivem, os prazos não) — o mesmo já vale para as playlists.

### 5. Reproduza

Selecione uma TV na lista, escolha um vídeo ou imagem e mande reproduzir — ou monte uma playlist na seção **Playlists**, logo abaixo, e inicie a playlist na TV escolhida.
