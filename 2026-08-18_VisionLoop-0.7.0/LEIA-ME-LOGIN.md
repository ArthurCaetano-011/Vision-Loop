...
>
> As migrações **já rodaram no seu banco** e por isso foram **removidas do
> pacote** na 0.6.9-fix2 — elas eram scripts de uma vez só, sem uso depois de
> executados (mesma limpeza feita antes com o `schema-migracao-0.4.sql`).
> O texto mais abaixo ainda cita esses arquivos porque é o histórico das
> entregas anteriores; eles não estão mais aqui e não precisam estar.
>
> O que **continua no pacote** é o **`schema-contas.sql`**: ele não é
> migração, é a planta completa do banco, usada só para montar tudo do zero
> (banco novo, troca de provedor, ambiente de teste). Já está atualizado com
> a coluna `expira_em` do prazo de validade.
