...
function connect() {
  ...
  ws.onmessage = (e) => {
    ...
    if (msg.type === 'your_code') {
      ...
      // Toda reconexão vira uma entrada NOVA no Map em memória do servidor
      // (mesma linha do banco, `code` novo) — sem isso, uma TV que já estava
      // em tela cheia antes de uma queda de rede voltaria mostrando
      // "Tela cheia" no painel, errado, até alguém mexer no botão.
      avisarStatusFullscreen();
    }
...
// Estado REAL de tela cheia, confirmado pelo próprio navegador da TV — cobre
// tanto o pedido não confirmado no controle remoto (requestFullscreen() só
// dispara com um clique de verdade em cima do #fsBtn; sem isso o evento nunca
// chega e o painel nunca acha que está em tela cheia) quanto sair sem passar
// pelo painel (controle remoto, tecla de voltar da TV etc. — o navegador
// dispara o evento de qualquer forma). O painel usa isso pra manter o botão
// "Tela cheia" certo, em vez de confiar só no que ele mesmo mandou pedir.
function avisarStatusFullscreen() {
  const emTelaCheia = !!(document.fullscreenElement || document.webkitFullscreenElement);
  if (ws && ws.readyState === 1) {
    ws.send(JSON.stringify({ type: 'fullscreen_status', fullscreen: emTelaCheia }));
  }
}
document.addEventListener('fullscreenchange', avisarStatusFullscreen);
document.addEventListener('webkitfullscreenchange', avisarStatusFullscreen);
