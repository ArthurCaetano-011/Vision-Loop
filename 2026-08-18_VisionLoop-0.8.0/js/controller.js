...
function connect() {
  ...
  ws.onmessage = (e) => {
    ...
    if (msg.type === 'tv_list') {
      // "Tela cheia" é um estado só desta aba do painel (o servidor não sabe
      // se a TV está mesmo em tela cheia) — precisa sobreviver a cada
      // atualização de tv_list. Chaveado por tvRowId, não por code: code
      // muda a cada reconexão da TV e não existe enquanto ela está offline.
      const fsMap = {};
      tvs.forEach(t => { if (t.fullscreen) fsMap[String(t.tvRowId)] = true; });
      tvs = msg.tvs.map(t => ({ ...t, fullscreen: !!fsMap[String(t.tvRowId)] }));
      renderTvList();
      renderPlTvSelect();
    }
    if (msg.type === 'media_changed') {
      // Um arquivo venceu e saiu sozinho (varredura de validade no servidor,
      // a cada 60s) — recarrega o uso de armazenamento pra refletir isso na
      // hora, sem precisar esperar a próxima ação manual do usuário.
      loadStorageUsage();
    }
...
// Tela cheia é um comando meio diferente dos outros três: não existe
// confirmação nenhuma vinda da TV dizendo se ela está mesmo em tela cheia
// (o navegador da TV não avisa isso pelo WebSocket), então o botão alterna
// um estado só LOCAL (guardado em tv.fullscreen, sobrevive a cada tv_list —
// ver o handler de 'tv_list' acima). "Ligar" mostra na TV o prompt pra
// confirmar tela cheia (precisa de alguém com o controle remoto dela pra
// apertar OK — navegador nenhum deixa um comando remoto forçar tela cheia
// sem interação de verdade); "desligar" sai de vez, sem precisar de prompt.
function tvToggleFullscreen(tvRowId, code, event) {
  ...
  if (!tv) return;
  const ligar = !tv.fullscreen;
  ws.send(JSON.stringify({ type: ligar ? 'enter_fullscreen' : 'exit_fullscreen', code }));
  tv.fullscreen = ligar;
  renderTvList();
}
