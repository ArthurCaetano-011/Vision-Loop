# VisionLoop_Logins_0.1 — Login multi-usuário (Fase 1)
...
- Login único por e-mail/senha, com dois papéis (`adm` / `cliente`) — mesma
  tela, o servidor decide o que liberar depois.
...
| `package.json` | Alterado — dependências novas `pg` e `bcrypt`, versão `0.1.3-logins-0.1` |
...
6. **Criar sua conta ADM:** gere o hash da sua senha
   (`node -e "require('bcrypt').hash('SUA_SENHA', 10).then(console.log)"`,
   já com o `npm install` feito) e rode o `INSERT` comentado no fim do
   `schema-contas.sql`, substituindo o hash e o e-mail.
