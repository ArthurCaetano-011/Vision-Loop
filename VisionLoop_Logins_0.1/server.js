...
const server = http.createServer((req, res) => {
  ...
  // ---------- AUTENTICAÇÃO ----------
  // Login único (adm/cliente na mesma tabela `contas`, diferenciados pelo
  // campo `role`). Sem cadastro público — a única forma de uma conta existir
  // é o ADM criando pelo painel (Fase 2). Ver lib/auth.js para as regras de
  // senha/sessão/licença.
  ...
  if (req.method === "POST" && urlPath === "/login") {
    ...
    req.on("end", async () => {
      ...
      try {
        const resultado = await auth.autenticar(parsed.email, parsed.senha);
        if (resultado.erro) {
...
wss.on("connection", (ws, req) => {
  ...
  // TVs não têm sessão (pareamento por conta é a Fase 3), então isso não
  // afeta o fluxo delas.
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "controller_connect") {
      // Conexão direta: sem código de sala. Todo controlador logado já
      // enxerga e controla todas as TVs conectadas ao servidor — o login
      // aqui só garante que É ALGUÉM LOGADO, ainda não isola por conta
      // (isso vem na Fase 3, junto do pareamento de TV por conta).
