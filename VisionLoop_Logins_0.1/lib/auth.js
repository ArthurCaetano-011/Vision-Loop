...
async function autenticar(email, senha) {
  ...
  const { rows } = await pool.query("SELECT * FROM contas WHERE email = $1", [emailNormalizado]);
  const conta = rows[0];
  // Mesma mensagem genérica pros dois casos (e-mail não existe / senha
  // errada) — não dar pista de qual dos dois estava errado.
  if (!conta) return { erro: "E-mail ou senha inválidos." };
