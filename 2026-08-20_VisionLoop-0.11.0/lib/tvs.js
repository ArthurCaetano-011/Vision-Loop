...
// Todas as TVs já pareadas (com dono), pra montar a lista persistente do
// painel do controlador. Diferente do Map `tvs` em memória (server.js), que
// só sabe quem está conectado agora, esta consulta traz TODA TV vinculada,
// esteja conectada ou não — é o que permite uma TV pareada continuar
// aparecendo na lista (com status offline) mesmo desligada ou sem internet.
// O JOIN com `contas` traz o nome do negócio dono da TV — usado no painel do
// ADM, que vê TVs de várias contas ao mesmo tempo e precisa saber de quem é
// cada uma (pra um cliente comum, sempre vai ser a própria conta dele).
async function listarTvsPareadas() {
  const { rows } = await pool.query(
    `SELECT tvs.id, tvs.nome, tvs.conta_id, contas.nome_negocio AS conta_nome
     FROM tvs
     JOIN contas ON contas.id = tvs.conta_id
     WHERE tvs.conta_id IS NOT NULL
     ORDER BY tvs.nome`
  );
  return rows;
}
