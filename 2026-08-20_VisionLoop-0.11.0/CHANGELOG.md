...
Novo: **precificador automático das tabelas de carne**. Agora é só enviar o
.txt exportado da balança pelo mesmo botão de "adicionar vídeo/imagem" dentro
de uma playlist — o VisionLoop lê os preços, preenche as 7 tabelas (Bovinos,
Suínos, Linguiças e Aves) e já adiciona as 7 imagens na ordem de reprodução
da playlist que estava aberta, prontas pra revisar, reordenar e salvar como
qualquer outro upload. Exibir numa TV continua sendo sempre uma escolha
manual do cliente — este recurso não cria playlist nenhuma sozinho nem manda
nada pra nenhuma TV por conta própria.

### Como funciona

Dentro de uma playlist, o botão de enviar arquivo passou a aceitar `.txt`
além de vídeo/imagem. Ao soltar (ou selecionar) um `.txt`, o VisionLoop
reconhece que não é mídia — é a exportação da balança — e segue um caminho
totalmente diferente do upload comum:

1. Lê o `.txt` (formato de largura fixa da balança) e extrai nome e preço de
   cada item cadastrado.
2. Para cada uma das 60 linhas das 7 tabelas-base (Bovinos ×3, Suínos,
   Linguiças ×2, Aves — as artes prontas, com nome de cada corte e foto,
   ficam em `assets/tabelas/`), busca o preço certo. Essa busca usa um
   mapeamento **fixo** linha→nome-no-txt (não é uma adivinhação por
   aproximação de texto): a ordem das tabelas nunca muda, então o preço certo
   de cada linha já está resolvido de antemão — inclusive os casos em que o
   nome bonito da tabela não bate literalmente com o nome cru do txt (ex.:
   "Pé de Frango" vem de "PE E PESCOSSO DE FRANGO", não de "PE DE FRANGO",
   que é outro item mais barato). Quando o mesmo item aparece duas vezes no
   txt com preços diferentes, vale sempre o mais caro.
3. Desenha o preço encontrado em cima de uma cópia de cada tabela-base, já no
   tamanho e posição certos, e salva como uma imagem normal (nome de arquivo
   FIXO — a próxima vez que um `.txt` for enviado, essas 7 imagens são
   substituídas pelas novas, sem acumular lixo no armazenamento).
4. Devolve os 7 nomes de arquivo pro controlador, que os adiciona na ordem de
   reprodução da playlist que já estava aberta — exatamente como qualquer
   outro upload. Nada é salvo automaticamente (ainda é preciso clicar em
   **Salvar**) e nenhuma TV é acionada sozinha — quem decide quando e onde
   exibir continua sendo sempre o cliente, escolhendo na tela.

Se algum item da tabela não for encontrado no `.txt` enviado (ex.: a loja não
cadastrou aquele corte na balança daquela vez), a célula de preço
correspondente fica em branco e o painel avisa quais itens ficaram faltando
— o resto da tabela sai normalmente.

### Preenche a tela toda (16:9)

As 7 artes originais (2245×1587) são um pouco mais "quadradas" que a tela de
uma TV (quase sempre 16:9) — exibidas do jeito que vieram do Canva, sobrava
tarja preta nas laterais. Agora cada imagem final sai já em 16:9: a coluna de
nome+preço continua exatamente igual (nem um pixel de diferença, mesmo
tamanho de fonte), só a foto do lado direito é ampliada levemente (sem
esticar/deformar — é um corte com zoom, não uma distorção) até fechar a
largura que falta. O resultado toca em tela cheia em qualquer TV 16:9, sem
faixa preta.

### Detalhes técnicos

Todo o texto do preço é desenhado via SVG com a fonte (DejaVu Sans Bold)
embutida em base64 dentro do próprio SVG (`lib/precificador.js`) — não
depende de nenhuma fonte instalada no servidor de produção, então o preço sai
igual em qualquer ambiente. Nova dependência: `sharp` (composição de
imagem), adicionada ao `package.json`.

---

## 0.9.0 — 18/08/2026

Novo: **cada TV mostra de qual conta ela é** (pro ADM), e o botão de **tela
cheia agora reflete o estado real da TV**, não mais uma suposição do painel.
Mais dois ajustes: **playlists se atualizam sozinhas** quando um item vence,
e **só dá pra subir um arquivo por vez**.

### TV mostra a conta dona

O ADM vê TVs de várias contas juntas na mesma lista — antes não dava pra
saber de quem era cada uma sem abrir os detalhes. Agora cada card de TV traz
um selo **🏢 Nome da conta**. Pra uma conta comum (não-ADM), que só vê as
próprias TVs, o selo não aparece — seria óbvio e repetitivo. Vem de um JOIN
novo em `listarTvsPareadas()` (`tvs` com `contas`, pelo `conta_id`).

### Tela cheia confirmada de verdade

Antes, clicar em "Tela cheia" mudava o botão na hora, só que era uma
suposição: o comando manda a TV mostrar um prompt ("aperte OK no controle
remoto pra confirmar" — nenhum navegador deixa forçar tela cheia sem
interação real), e o painel não tinha como saber se alguém realmente
confirmou. Se ninguém apertasse OK, ou se a TV saísse da tela cheia sozinha
(botão de voltar do controle remoto, por exemplo), o botão ficava errado até
a próxima ação.

Agora a própria TV avisa o servidor sempre que entra ou sai de tela cheia de
verdade (`fullscreenchange` do navegador, novo em `js/tv.js`), o servidor
guarda esse estado por TV e manda no `tv_list`, e o painel só troca o botão
quando essa confirmação chega — nunca mais na hora do clique. Cobre os três
casos: prompt nunca confirmado, saída pelo controle remoto, e reconexão da
TV depois de uma queda de rede (o estado real é reenviado assim que ela volta
a se identificar).

### Playlists não precisam mais de F5 quando um vídeo vence

A varredura de validade (a cada 60s) já tirava o arquivo vencido de toda
playlist que o usava — mas a tela de playlists do painel só refletia isso
depois de um F5 ou de trocar de playlist e voltar. Agora o mesmo aviso
`media_changed` que já atualizava a biblioteca (ver 0.8.0) também recarrega
as playlists, e se a playlist afetada estiver **aberta em modo visualização**
naquele momento, ela é redesenhada na hora. O formulário de **edição** (que
trabalha em cima de uma cópia local ainda não salva) não é mexido por esse
aviso, de propósito — sincronizar às cegas ali arriscaria apagar uma edição
em andamento.

### Um arquivo por vez

O seletor de arquivo da playlist não aceita mais escolher vários de uma vez
(tirado o `multiple` do campo), e soltar um punhado de arquivos arrastando
também é barrado, com aviso pra soltar só um. Os uploads já eram enviados um
de cada vez por trás dos panos — a mudança é não deixar mais **enfileirar**
vários no mesmo envio.

**Arquivos alterados:** `lib/tvs.js` (JOIN com `contas` em
`listarTvsPareadas`), `server.js` (`contaNome`/`fullscreen` no `tv_list`,
handler novo de `fullscreen_status`, `fullscreen: false` na criação da TV em
memória), `js/tv.js` (ouvinte de `fullscreenchange`/`webkitfullscreenchange`,
aviso ao reconectar), `js/controller.js` (selo de conta, botão de tela cheia
sem estado otimista, `loadPlaylists()` no `media_changed` e redesenho da
playlist em visualização, `handleFiles()` rejeitando mais de um arquivo),
`controller.html` (`multiple` removido do input de arquivo), `css/controller.css`
(estilo do selo de conta).

### Como testei

De novo com um `server.js` real contra Postgres/WebSocket simulados em
memória: login de uma conta cliente e de uma ADM, TV pareada com a conta
cliente aparecendo com `contaNome` correto tanto pro ADM quanto pro cliente
(o campo vem sempre, quem decide esconder é a interface); ciclo completo de
tela cheia — pedido do painel não muda o estado sozinho, só quando a TV
confirma (`fullscreen_status: true`), volta a `false` quando a TV avisa que
saiu por conta própria, e cai pra `false` também quando ela desconecta; e
confirmação de que o JOIN novo em `listarTvsPareadas()` não quebrou o
`media_changed` da 0.8.0 (varredura periódica real de 60s, mídia some do
banco e do disco, aviso chega no painel).

---

## 0.8.0 — 18/08/2026

Novo: **tela cheia por TV**, **TVs pareadas continuam na lista quando ficam
offline**, e a biblioteca do painel **se atualiza sozinha quando um arquivo
vence**. Também saiu a grade "Vídeos disponíveis" de dentro do formulário de
playlist.

### Tela cheia

Cada TV no painel agora tem um botão **⛶ Tela cheia** ao lado de
Pausar/Retomar/Parar. O comando (`enter_fullscreen`/`exit_fullscreen`) e o
código do lado da TV já existiam desde antes — só faltava o botão no
controlador. Fica desabilitado, como os outros três, enquanto a TV está
offline.

### TVs offline continuam na lista

Antes, a lista de TVs do painel vinha só das conexões abertas no momento
(`Map` em memória) — desligar a TV a fazia sumir da lista, mesmo continuando
pareada à conta. Agora a lista parte do banco (`tvs`, toda TV com
`conta_id`), e cada linha é cruzada com as conexões vivas: pareada e
conectada aparece normal; pareada e desconectada aparece apagada, com o selo
**Offline**, sem os detalhes de "tocando agora" e com os botões de controle
desabilitados — mas continua lá, dá pra desparear, e some da lista assim que
alguém desparear de verdade.

Antes disso, cada TV era identificada no painel pelo `code` de sessão
(efêmero, muda a cada reconexão e não existe pra uma TV desligada). Agora é
pelo `tvRowId` (o id dela no banco, estável), corrigido em toda a tela de
TVs do `js/controller.js` (seleção, tela cheia, desparear).

### Biblioteca some da mídia vencida sozinha

A varredura de validade (a cada 60s, já existia desde a 0.6.9) apagava o
arquivo do disco, tirava ele das playlists e avisava as TVs — mas nunca
avisava o **painel**, que só percebia num "↻ atualizar" manual ou recarregando
a página. Agora o servidor manda um `media_changed` pros controladores da
conta afetada (e pro ADM) assim que a varredura mexe em algo, e o painel
recarrega a biblioteca e o uso de armazenamento na hora.

### Saiu a grade "Vídeos disponíveis"

A barra que mostrava todos os vídeos já enviados, pra escolher e adicionar a
uma playlist, foi removida do formulário de playlist. Enviar um arquivo
continua colocando ele direto na playlist que está sendo editada — isso não
mudou. **Efeito colateral:** como o botão de excluir vídeo só existia dentro
dessa grade, não sobrou nenhum jeito de excluir um arquivo já enviado pela
interface (a função que faz a exclusão, `deleteVideo`, continua no código,
só sem nada que a chame). Se isso fizer falta, dá pra devolver um atalho de
exclusão em outro lugar — por exemplo, na lista "Ordem de reprodução" de cada
playlist.

**Arquivos alterados:** `server.js` (`sendTvListToController`/
`broadcastTvList` reescritos pra ler de `tvs` no banco, `media_changed` na
varredura de validade), `lib/tvs.js` (`listarTvsPareadas`), `js/controller.js`
(botão de tela cheia, lista de TVs por `tvRowId`, handler de `media_changed`,
remoção de `renderPlVideoGrid`/`togglePlVideo`/`loadMediaMeta`/
`expiryBadgeHtml`), `controller.html` e `css/controller.css` (marcação e
estilo da TV offline, remoção da grade).

### Como testei

Sem TV física nem banco de produção neste ambiente: rodei o `server.js` de
verdade (mesmo código, zero mock de lógica) contra um Postgres e um
WebSocket simulados em memória, e um roteiro ponta a ponta automatizado:

- Login, pareamento de uma TV por código, e `tv_list` mostrando ela
  `connected: true` com `tvRowId` estável.
- Comando de tela cheia do painel chegando na TV certa.
- TV desconectando: `tv_list` continua trazendo ela, agora
  `connected: false`, `code: null`, mesmo `tvRowId` de antes.
- TV reconectando com o mesmo aparelho: volta a aparecer online, com o
  **mesmo** `tvRowId` (não vira uma TV nova).
- Um arquivo vencendo (validade no passado): a varredura de 60s apaga ele do
  banco e do disco sozinha, e o controlador conectado recebe o
  `media_changed` na hora, sem precisar atualizar a página.

---

## 0.7.0 — 18/08/2026

Novo: **a TV baixa a playlist inteira antes de começar a tocar.**

Até aqui, cada item da playlist só começava a baixar perto da vez dele (o
preload do "próximo" item já existia, mas só ~8s antes do fim do atual). Numa
conexão de Wi-Fi ruim, isso podia deixar a troca entre um item e outro mais
lenta ou instável. Agora, ao receber uma playlist nova, a TV primeiro baixa
(só os bytes, sem decodificar) o arquivo de **todos** os itens em segundo
plano — mostrando uma tela "Carregando mídia... X/Y" com barra de progresso
— e só começa a exibir depois que tudo estiver pronto (ou depois de 90s, pra
não travar pra sempre se algum arquivo específico falhar em baixar).

### O que NÃO mudou (de propósito)

A decodificação continua acontecendo **um vídeo por vez**, exatamente como
antes — o double buffer e a janela de preload tardio (`PRELOAD_LEAD_MS`)
seguem intactos. Isso é importante porque várias Smart TVs de entrada têm um
único decodificador H.264 por hardware; decodificar dois vídeos ao mesmo
tempo é o que causava os travamentos que o double buffer já foi desenhado
para evitar. Só a etapa de *download* passou a ser adiantada — a de
*decodificação/exibição* não mudou em nada.

### Comportamento em cenários de corrida

- **Comando novo chega no meio do carregamento** (outra playlist, um "play"
  avulso, um "parar", ou a TV ser despareada): o carregamento em andamento é
  descartado silenciosamente — só o pedido mais recente vale, sem risco de
  tocar conteúdo desatualizado ou a tela de carregamento ficar presa.
- **Playlist editada no controlador enquanto a TV ainda está no carregamento
  inicial** (antes do primeiro item aparecer na tela): tratado como se a TV
  ainda não tivesse começado nada — recomeça do zero com a versão mais nova,
  em vez de tentar aplicar a edição "por cima" de uma playlist que nem chegou
  a ser exibida. Depois que a playlist já está tocando de verdade, edições
  continuam sendo aplicadas em tempo real, sem interromper o item atual (ver
  `aplicarPlaylistAtualizada`, sem mudanças de comportamento aqui).

**Arquivos alterados:** `js/tv.js` (pré-carregamento em blob local, e as
funções de reprodução/preload do próximo item passaram a usar esses blobs em
vez de baixar de novo) e `tv.html` (nova tela `#loadingScreen`, escondida o
resto do tempo). Nenhuma mudança de servidor, rota ou banco de dados.

### Como testei

Sem acesso a uma TV física nem ao banco de produção neste ambiente: testei a
lógica isolada, num navegador automatizado, servindo os arquivos do projeto
localmente e chamando as mesmas funções que a TV usa quando o controlador
manda algo:

- Playlist com 2 itens: tela de carregamento aparece com progresso, nada é
  exibido até os dois arquivos terminarem de baixar, e a reprodução começa
  certa a partir do primeiro item.
- `update_playlist` chegando **depois** que o primeiro item já está na tela:
  aplicado em tempo real, sem reiniciar a reprodução (confirma que o novo
  `playlistPlaybackStarted` não interfere no caminho já existente).
- `update_playlist` chegando **durante** o carregamento inicial (antes de
  qualquer item aparecer): confirma que só a versão mais nova chega a tocar.
- Playlist nova chegando no meio do carregamento de outra: só a mais recente
  toca.
- "Parar" chegando no meio do carregamento: aborta sem tocar nada indevido e
  libera a memória dos arquivos já baixados.
