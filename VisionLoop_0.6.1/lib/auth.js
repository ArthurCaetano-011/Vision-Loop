...
// Conta cliente marcada como licenca_permanente também nunca bloqueia por
// validade (ver painel Contas → "Licença permanente"). Conta cliente comum
// com licença vencida, sem data definida, ou suspensa manualmente
// (ativa=false), não consegue logar nem manter uma sessão já aberta.
function licencaValida(conta) {
  if (!conta.ativa) return false;
  if (conta.role === "adm") return true;
  if (conta.licenca_permanente) return true;
