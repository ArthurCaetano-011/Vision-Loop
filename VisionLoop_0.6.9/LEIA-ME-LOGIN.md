> ## ⚠️ ANTES DE SUBIR A 0.6.9: rode uma migração no Neon
>
> A 0.6.9 acrescenta o **prazo de validade** dos arquivos, e isso precisa de
> uma coluna nova na tabela `midia`. Abra o **SQL Editor do Neon**, cole o
> conteúdo de **`schema-migracao-0.6.9.sql`** e rode **uma vez**, antes do
> deploy. É seguro (só `ADD COLUMN IF NOT EXISTS`) e não mexe em nenhum dado
> que já existe — tudo que está lá hoje fica com validade vazia, ou seja,
> **sem prazo**, exatamente como sempre foi.
>
> Sem rodar isso, o envio de arquivos e a listagem da biblioteca quebram,
> porque o servidor passa a ler e gravar essa coluna.
