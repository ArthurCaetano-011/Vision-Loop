...
// A coluna `midia.expira_em` vem da migração schema-migracao-0.6.9.sql. Se o
// banco ainda não a tem (deploy feito antes de rodar a migração), o Postgres
// devolve o erro 42703 ("column does not exist"). Sem este cuidado, esse erro
// derrubava a LISTAGEM da biblioteca e — no modo R2 — o próprio upload: quem
// atualizasse o app sem rodar a migração ficava sem conseguir subir arquivo.
//
// Com o cuidado abaixo, o app funciona exatamente como na 0.6.8 (todo arquivo
// sem prazo) até a migração ser rodada, e volta a honrar a validade sozinho
// em até um minuto depois — sem precisar reiniciar o servidor.
...
function marcarColunaAusente(err) {
  ...
  if (primeiraVez) {
    console.error(
      ...
      "   O prazo de validade dos arquivos fica DESLIGADO até você rodar\n" +
      "   schema-migracao-0.6.9.sql no SQL Editor do Neon. O resto do app\n" +
      "   (upload, biblioteca, playlists, TVs) continua funcionando normal.\n"
    );
  }
  return true;
}
