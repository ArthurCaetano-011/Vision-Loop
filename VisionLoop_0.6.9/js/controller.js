...
function loadVideos(btn) {
  ...
  fetch('/videos-list')
    ...
    .then(list => {
      videos = list;
      // As validades vêm numa chamada separada; o selo só é desenhado depois
      // que elas chegam, então a grade é redesenhada nas duas etapas.
      renderPlVideoGrid();
      return loadMediaMeta().then(() => renderPlVideoGrid());
    })
    .catch(err => {
...
function renderTvList() {
  ...
  list.innerHTML = tvs.map(tv => {
    ...
    <div class="tv-card ${selectedTvCode === tv.code ? 'selected' : ''}" onclick="selectTv('${tv.code}')">
      <button class="tv-unpair-btn" onclick="despairTv('${tv.code}', event)" title="Desparear esta TV desta conta">Desparear</button>
      ...
      ${selectedTvCode === tv.code ? controlesDaTvHtml(tv) : ''}
    </div>`;
  }).join('');
}

// Controles da TV clicada. Cada botão carrega o CÓDIGO da própria TV e manda o
// comando só para ela — o servidor entrega a mensagem exclusivamente para
// aquele aparelho (tvs.get(code)), então pausar/parar/retomar uma TV nunca
// alcança as outras, mesmo com várias pareadas na mesma conta.
function controlesDaTvHtml(tv) {
  const temConteudo = !!(tv.video || tv.playlist);
  const c = tv.code;
  return `
      <div class="tv-controls" onclick="event.stopPropagation()">
        <button class="btn btn-green btn-sm" onclick="tvResume('${c}', event)" ${temConteudo && tv.paused ? '' : 'disabled'}>▶ Retomar</button>
        <button class="btn btn-ghost btn-sm" onclick="tvPause('${c}', event)" ${temConteudo && !tv.paused ? '' : 'disabled'}>⏸ Pausar</button>
        <button class="btn btn-red btn-sm" onclick="tvStop('${c}', event)" ${temConteudo ? '' : 'disabled'}>⏹ Parar</button>
      </div>`;
}

// Um único caminho para os três comandos, sempre com o código explícito da TV
// — nada aqui depende de "qual TV está selecionada" na hora do envio, que é o
// que poderia fazer um clique afetar a TV errada.
...
// `code` vem do botão no card da TV. Sem argumento, cai no comportamento
// anterior (a TV atualmente selecionada) — a lógica de desemparelhamento em si
// segue exatamente a mesma.
function despairTv(code, event) {
  if (event) event.stopPropagation(); // não deixa o clique selecionar o card
  const alvo = code || selectedTvCode;
  if (!alvo) return;
  const tv = tvs.find((t) => t.code === alvo);
...
// O que o servidor sabe sobre prazos: { "arquivo.mp4": { expiresAt } }.
// Arquivo que não aparece aqui é arquivo sem prazo — fica até ser excluído.
let mediaMeta = {};
...
// Selo no canto do cartão do arquivo que tem prazo pra sair do ar. Fica em
// âmbar quando falta menos de 24h, pra o vencimento não pegar de surpresa.
function expiryBadgeHtml(name) {
  ...
  return `<div class="${classe}" title="Sai do ar em ${formatExpiry(entry.expiresAt)}">⏳ ${formatExpiry(entry.expiresAt)}</div>`;
}
...
function handleFiles(fileList, listId, onUploaded) {
  ...
  // A escolha vale pra leva inteira daquele envio (um arquivo ou vários).
  // Cancelar aqui cancela o envio todo — nada sobe.
...
// formulário de playlist: além de entrar na biblioteca, já é incluído na ordem
// de reprodução — que é o que se espera de "enviar para esta playlist".
