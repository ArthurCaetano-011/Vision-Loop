...
function handleFiles(fileList, listId, onUploaded) {
  ...
  processUploadQueue(queue, listId, onUploaded);
}
...
async function processUploadQueue(files, listId, onUploaded) {
  for (const file of files) {
    const finalFile = await resizeImageIfNeeded(file);
    await uploadFile(finalFile, listId, onUploaded);
  }
}

function uploadFile(file, listId, onUploaded) {
  return new Promise((resolve) => {
...
function showPlView(pl) {
  ...
  document.getElementById('plViewCount').textContent = vazia
    ? 'Nenhum item ainda — clique em Editar para adicionar'
  ...
  if (vazia) {
    ol.innerHTML = `<div style="color:var(--muted); font-size:13px; padding:12px 0;">
      Esta playlist ainda está vazia. Use <strong>✏️ Editar</strong> para enviar arquivos ou escolher os que já estão na biblioteca.
    </div>`;
  } else {
