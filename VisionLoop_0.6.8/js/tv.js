...
function connect() {
  ...
  ws.onmessage = (e) => {
    ...
    if (msg.type === 'stop') { pausado = false; stopMedia(); hidePlBadge(); playlistMode = false; currentPlaylist = null; }
...
function aplicarPlaylistAtualizada(playlist) {
  ...
  // Não estava em modo playlist (ou a TV está parada): começa do começo.
  ...
  if (!itens.length) {
    ...
    playlistMode = false; currentPlaylist = null; pausado = false;
    return;
  }
