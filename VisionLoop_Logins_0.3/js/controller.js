...
function renderContaList() {
  ...
  list.innerHTML = contas.map((c) => {
    ...
    let licencaTag = '';
    if (c.role !== 'adm') {
      if (!c.licenca_expira_em) licencaTag = '<span class="tag tag-amber">Sem validade</span>';
      else if (new Date(c.licenca_expira_em).getTime() < Date.now()) licencaTag = '<span class="tag tag-amber">Licença vencida</span>';
    }
